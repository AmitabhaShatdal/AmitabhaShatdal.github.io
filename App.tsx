...
type Page = 'home' | 'privacy' | 'terms' | 'about' | 'contact' | 'faq';
...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'faq':
        return <FAQ onBack={() => handleNavigate('home')} />;
      default:
  ...
  const renderHomePage = () => (
    <>
      ...
      {/* Loading State - Reduced ad density: single centered ad */}
      {(status === AnalysisStatus.SEARCHING || status === AnalysisStatus.ANALYZING) && (
        <div className="w-full max-w-2xl mx-auto py-10 animate-in fade-in duration-700">
           {/* Status Text */}
